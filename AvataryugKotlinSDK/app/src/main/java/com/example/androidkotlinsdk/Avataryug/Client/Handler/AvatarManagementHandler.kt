...
class AvatarManagementHandler (private val apiBase: Base)
{
    ...
    fun getUserAvatarAllData(listener: OnGetUserAvatarAllDataListener)
    {
        apiBase.callApi(object : Base.OnApiResultListener {
            override fun onResult(response: Any) {
                if (response is GetUserAvatarAllDataResult) {
                    listener.onOnGetUserAvatarAllDataResult(response)
                } else {
                    listener.onOnGetUserAvatarAllDataError(Exception("Invalid response type"))
                }
            }

            override fun onError(error: Exception) {
                listener.onOnGetUserAvatarAllDataError(error)
            }
        })
    }
     ...
     */
    interface OnGetUserAvatarAllDataListener
    {
        fun onOnGetUserAvatarAllDataResult(result: GetUserAvatarAllDataResult)
        fun onOnGetUserAvatarAllDataError(error: Exception)
    }
        ...
        apiBase.callApi(object : OnApiResultListener {
            override fun onResult(response: Any) {
                if (response is GrantAvatarPresetItemsToUserResult) {
                    listener?.onGrantAvatarPresetItemsToUserResult(response )
                } else {
    ...
    class GetUserAvatarAllData(private  val avatarID: String,private val platform: String) : Base
    {
        override fun callApi(listener: Base.OnApiResultListener)
        {
            GlobalScope.launch(Dispatchers.IO)
            {
                ...
                {
                    val apiInstance = AvatarManagementApi()
                    val result: GetUserAvatarAllDataResult = apiInstance.getUserAvatarAllData (avatarID,platform)
                    listener.onResult(result)
                }
                catch (e: ClientException)
                {
